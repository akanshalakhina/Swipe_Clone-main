...
import { Plus, Search, Package, Edit, Trash2, AlertTriangle, X } from 'lucide-react'
...
export default function InventoryPage() {
  ...
  const [newProduct, setNewProduct] = useState({ name: '', price: '', stock: '', category: 'General', sku: '' })
  
  const { products, isLoading, isError, createProduct, deleteProduct } = useProducts()
  ...
  const handleAddProduct = async (e) => {
    ...
    try {
      ...
      setNewProduct({ name: '', price: '', stock: '', category: 'General', sku: '' })
    } catch (err) {
    ...
    <div className="p-6 space-y-5">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">{products.length} products • Stock value: ₹{totalValue.toLocaleString()}</p>
        </div>
        <button 
            ...
            <tbody className="divide-y divide-gray-50">
              {filtered.map((p, i) => (
                ...
                >
                  <td className="py-4 px-5">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <div className="text-[11px] text-gray-400">{p.sku || 'No SKU'}</div>
                      </div>
                    </div>
                  </td>
                  ...
                  <td className="py-4 px-5 text-sm font-bold text-gray-900 text-right">₹{Number(p.price).toLocaleString()}</td>
